...
#     index_with_extensions.html (best for offline use)
#  ENVIRONMENT:
#    a linux bash shell (i.e. GNU bash 5.2.2, perl v5.36.0, sed 4.8)

# INSTRUCTIONS
# if you want .html extensions on books (best for offline use):
#   at the end of this file, comment out the line that removes extensions,
#     and uncomment the line that changes .htm extensions to html
#   use index_with_extensions.html as your index.html file

# run setup.sh (this file) in a clean directory that has engwebp_html.zip

# copy all outputted html files to a subdirectory named "book"
# copy style.css, caslon.ttf, and index.html to the parent
...
# genesis 5:10
sed -i 's/Enos lived after his begetting Cainan, eight hundred fifteen years/Enos lived after his begetting Cainan, seven hundred fifteen years/' genesis.htm
...
# "I was" supported by Lamsa Bible and Anderson New Testament
...
# some people do not prefer the messiah's name translated as "jesus";
# some people prefer "yahushua", "yahshua", "yeshua", or something similar.
# therefore, the way it is translated can be changed here.

# here is an argument why "jesus" may be a reasonable translation:
# https://christogenea.org/essays/yahshua-jesus-evolution-name

# besides that, after the tower of babel it is ok and healthy for people to have different languages. and this is english not hebrew.

# edit "jesus" to "yahushua"
#sed -i 's/Jesus/Yahushua/g' *.htm
#sed -i 's/JESUS/YAHUSHUA/g' *.htm
...
# the best practice for websites is to have no extension

# create a list
ls -1 *.htm | sed 's/\.htm//' > books2.txt

# change filenames from .htm to .html for offline use
# while read; do mv "${REPLY}".htm "${REPLY}".html; done < books2.txt

# remove filename extension for website use
while read; do mv "${REPLY}".htm "${REPLY}"; done < books2.txt
